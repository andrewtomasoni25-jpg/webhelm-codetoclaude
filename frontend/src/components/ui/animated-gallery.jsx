...
export const GalleryContainer = ({ children, className, style, ...props }) => {
  ...
  // Tilt lifts quickly, then the gallery is flat for the rest of the scroll
  const rotateX = useTransform(scrollYProgress, [0, 0.2], [60, 0]);
  const scale = useTransform(scrollYProgress, [0.1, 0.3], [1.1, 1]);
...
export const GalleryCol = ({
  ...
  const { scrollYProgress } = useContainerScrollContext();
  // Parallax spans the entire scroll, with a short flat intro while the tilt settles
  const y = useTransform(scrollYProgress, [0.2, 1], yRange);
